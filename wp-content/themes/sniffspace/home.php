...
$sniffspace_blog_title = get_the_title( get_option( 'page_for_posts' ) );

if ( ! $sniffspace_blog_title ) {
	$sniffspace_blog_title = __( 'Blog', 'sniffspace' );
}
?>

	<main id="primary" class="ss-blog-main">
		<section class="ss-blog-hero" aria-labelledby="ss-blog-title">
			<div class="ss-blog-container">
				<h1 id="ss-blog-title"><?php echo esc_html( $sniffspace_blog_title ); ?></h1>
				<nav class="ss-blog-breadcrumbs" aria-label="<?php esc_attr_e( 'Breadcrumb', 'sniffspace' ); ?>">
					...
					<span aria-current="page"><?php echo esc_html( $sniffspace_blog_title ); ?></span>
				</nav>
			</div>
		</section>

		<div class="ss-blog-container ss-blog-layout">
			<section class="ss-blog-posts-area" aria-label="<?php esc_attr_e( 'Blog posts', 'sniffspace' ); ?>">
				...
				if ( have_posts() ) :
					...
					the_posts_pagination(
						array(
							...
							'screen_reader_text' => esc_html__( 'Blog navigation', 'sniffspace' ),
						)
					);
				else :
